 ...
 * 重新提交整改派单
 */
public class ResubmitRectificationActivity extends BaseActivity {
                    ...
                    public void onVoiceRecordComplete(String voiceFilePath, int voiceTimeLength) {
                        ...
                        if (data.size() > 4) {
                            ThreadUtils.runOnMainThread(new Runnable() {
                                @Override
                                public void run() {
                                    MyToast.getInstands().toastShow(ResubmitRectificationActivity.this, "语音限制为5条以内");
                                }
                            });
                        } else {
    ...
    private void getRectificationDetail() {
        ...
        HttpManager.getInstance().getRectificationDetail(loginBean.getUser_id(), companyId, zId, new HttpEngine.OnResponseCallback<HttpResponse.RectificationDetail>() {
            ...
            public void onResponse(int result, @Nullable String retmsg, @Nullable HttpResponse.RectificationDetail data) {
                ...
                if (null != data) {
                    initEditInfo(data.getData());
                }
            }
        });
    }

    private void initEditInfo(RectificationBean data) {
        rectificationBean = data;
        if (rectificationBean != null) {
            //声音
            List<VoiceBean> voiceList = new ArrayList<>();
            List<String> voice = rectificationBean.getVoice_active_url();
            List<String> voiceLen = rectificationBean.getVoice_second();
            if (voice != null && voice.size() > 0) {
                for (int i = 0; i < voice.size(); i++) {
                    VoiceBean voiceBean = new VoiceBean();
                    voiceBean.setLen(Float.parseFloat(voiceLen.get(i)));
                    voiceBean.setPath(voice.get(i));
                    voiceList.add(voiceBean);
                }
                zgVoiceAdapter.setNewData(voiceList);
            }
            //项目信息
            if (!TextUtils.isEmpty(rectificationBean.getProject_id())) {
                project = new ProjectBean();
                project.setId(rectificationBean.getProject_id());
                project.setTitle(rectificationBean.getTitle());
                tvProject.setText(project.getTitle());
            }
            //图片
            if (null != rectificationBean.getImages_active_url() && rectificationBean.getImages_active_url().size() != 0) {
                List<String> images = rectificationBean.getImages_active_url();
            ...
            //截止日期
            if (!TextUtils.isEmpty(rectificationBean.getDeadline_time())) {
                deadline = rectificationBean.getDeadline_time();
                tvDeadline.setText(rectificationBean.getDeadline());
            }
            //所在位置
            if (!TextUtils.isEmpty(rectificationBean.getBuilding_id()) && !TextUtils.isEmpty(rectificationBean.getInside_id()) && !TextUtils.isEmpty(rectificationBean.getFloor_id())) {
                buildingNoBean = new BuildingNoBean();
                floorBean = new FloorBean();
                directionBean = new DirectionBean();
                buildingNoBean.setId(rectificationBean.getBuilding_id());
                floorBean.setId(rectificationBean.getFloor_id());
                directionBean.setId(rectificationBean.getInside_id());
                tvFloor.setText(rectificationBean.getBuilding_name() + "-" + rectificationBean.getFloor_name() + "-" + rectificationBean.getInside_name());
            }
            //详细位置描述
            if (!TextUtils.isEmpty(rectificationBean.getAddress_descrip())) {
                etLocation.setText(rectificationBean.getAddress_descrip());
            }
            //整改人
            if (!TextUtils.isEmpty(rectificationBean.getZgMan_id())) {
                staffBean = new StaffBean();
                staffBean.setId(rectificationBean.getZgMan_id());
            ...
            deadline = DateFormatUtils.formatDeadline2(24);
            tvDeadline.setText(DateFormatUtils.formatDeadline(24));
        }
    }
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()) {
            ...
            case R.id.activity_add_rectification_ll_project:
                ...
                IntentUtils.startActivityForResult(ResubmitRectificationActivity.this, ProjectSelectionActivity.class, bundle, Constants.REQUEST_CHOOSE_PROJECT);
                break;
            ...
            public void onClick(Dialog dialog, boolean confirm, String... param) {
                if (confirm) {
                    if (Constants.CUSTOMIZE.equals(param[0])) {
                        ...
                        deadline = DateFormatUtils.formatDeadline2(Integer.parseInt(param[0]));
                        tvDeadline.setText(DateFormatUtils.formatDeadline(Integer.parseInt(param[0])));
                        if (rectificationBean != null) {
    ...
    private boolean isNeedSave() {
        ...
        int imageSize = zgImagesAdapter.getItemCount();
        if (imageSize > 1) {
            isNeedSave = true;
        }
        int voiceSize = zgVoiceAdapter.getItemCount();
        if (voiceSize > 0) {
            isNeedSave = true;
        }
        if (project != null) {
